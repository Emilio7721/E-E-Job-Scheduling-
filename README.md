...
- **🧾 Timesheets & Paychex export** — **bi-weekly pay periods**; admins open each person to review their punches day by day, edit times, venue, job, mileage and read shift notes, then approve individually or all at once. An **automatic unpaid break** (30 minutes once a day reaches 5.5 worked hours by default, both configurable) is deducted **once per day** no matter how many punches it contains. Export produces a **Paychex SPI import CSV** (flexible layout, CRLF) containing **only approved hours** with the break already taken off — no rates, so Paychex applies each worker's configured rate. A second **detailed timesheet CSV** matches the Connecteam export column for column (shift hours, daily unpaid break, daily total, notes, mileage) for your own records.
